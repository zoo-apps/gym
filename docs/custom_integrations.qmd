...
def process_readme(integration_name):
    try:
        path = f'../src/gym/integrations/{integration_name}/README.md'
...
for folder_name in os.listdir("../src/gym/integrations/"):
    if folder_name in [path for name, path in sections]:
        # skip if already in sections
        continue
    if os.path.exists(f"../src/gym/integrations/{folder_name}/README.md"):
        # grab the first heading in README.md as the section name
        with open(f"../src/gym/integrations/{folder_name}/README.md", "r") as f:
            txt = f.read()
            matches = re.search(r'^# (.*)\n?', txt, flags=re.MULTILINE)
            if matches:
